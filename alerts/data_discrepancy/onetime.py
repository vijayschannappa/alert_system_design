...
ps = p_diff.pipe(
    cutoff_diffs, thresholds=[15], last_n_hour=[3], thresh_types=["median", "max"]
)

pe = add_data(ps, p_diff)
pe.to_csv("./data/scada_imdas_diff.csv", index=False)

# %%
min_ts = pe.TIMESTAMP.min().to_pydatetime()
max_ts = pe.TIMESTAMP.max().to_pydatetime()
plot_dir = Path(f"./plots/{min_ts:%d %b} {min_ts:%H:%M}_to_{max_ts:%H:%M}/")
plot_dir.mkdir(exist_ok=True)
pss_to_alert = pe.groupby("SUBSTATION_ID", sort=False).apply(
    save_plot, directory=plot_dir
)
summary_fn = f"./data/scada_imdas_diff_{min_ts:%Y-%m-%d %H:%M} to {max_ts:%H:%M}.csv"
